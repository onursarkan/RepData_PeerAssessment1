...
1. The total number of steps taken per dat is calculate:
```{r}
steps.date<-aggregate(steps~date,data=activityRecords,FUN=sum)
```

2. To understant the data, by following code, the histogram of daily step sums is produced.
```{r}
barplot(steps.date$steps, names.arg=steps.date$date, xlab="Days", ylab="NumOfsteps")
```

3. Mean and median is calculated by following code.
```{r}
...
1. With 5-minute intervals, average step numbers are summarized as a plot.
```{r}
steps.interval <- aggregate(steps~interval,data=activityRecords,FUN=mean)
plot(steps.interval,type="l")
```
2. Max step numbers are calculated.
```{r}
steps.interval$interval[which.max(steps.interval$steps)]
```

## Imputing missing values
1. Total number of missing values are calculated.

```{r}
sum(is.na(activityRecords$steps))
```

2. Devise a strategy: Actualy, if there is no data, I prefer to replace with means.
...
```
4. You can find the related plots.
```{r}
...
1. Day type variable is created.
```{r, cache=TRUE}
dType <- function(date) {
